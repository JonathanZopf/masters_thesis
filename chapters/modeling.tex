...
Given a value function $v(T)$ defined for any subset $T \subseteq P$, where $P$ denotes the set of all players, the value allotted to a player $m \in P$ is given by Equation~\ref{eq:shapley}.
...
where $\frac{|T|!\, (|P|-|T|-1)!}{|P|!}$ denotes the weighting factor, and $v(T \cup \{m\}) - v(T)$ denotes the marginal contribution provided by player $m$.
...
\begin{itemize}
	...
	\item \textbf{Symmetry}: Identical players who contribute the exact same amount to any coalition receive identical payouts (if $v(T \cup \{i\}) = v(T \cup \{j\})$ for all $T \subseteq P \setminus \{i, j\}$, then $\phi_i(v) = \phi_j(v)$).
...
\begin{equation}
	...
	\end{cases}
\end{equation}
estimates the typical shrinkage or amplification expected for treatments of size $|T|$, independently of $T$'s own realization. This reference level also permits a distinction between two notions of independence. \emph{Absolute additivity} means $\gamma(\mathbf{x},T)=1$, whereas \emph{reference-relative independence} means
\begin{equation}
	\gamma(\mathbf{x},T)=\bar\gamma(\mathbf{x},s,T).
\end{equation}
The latter does not assert that the joint effect is absolutely additive; it states that the treatment exhibits the level of non-additivity typical of equally sized reference treatments. Values below or above $\bar\gamma(\mathbf{x},s,T)$ indicate greater shrinkage or reinforcement, respectively, relative to that reference level.
...
This property follows under the assumptions of Math Box~\ref{ma:independence_behavior}. If a correlation group exhibits reference-relative independence, its payout coefficient equals the expected payout coefficient for groups of identical size,
\begin{equation}
