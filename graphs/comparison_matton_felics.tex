...
% Column Headers
\node[font=\sffamily\Large\bfseries\color{orange!80!black}] at (\colMatton, 1.5) {Matton et al.~\cite{mattonWalkTalkMeasuring2024}};
...
% TOP INPUT
% ==============================
\node[data] (orig) at (\colShared, 0) {\textbf{Original Question $\mathbf{x}$}};
...
\node[felics] (correl) at (\colFelics, -4.5) {Presume Interactions\\ \scriptsize Aux. LLM / Associations};
...
\node[felics] (felics_set) at (\colFelics, -7) {Determine Intervention Sets\\ \scriptsize (power set of candidate group)};
\draw[arrow] (correl.south) -- (felics_set.north);
\node[data, right] at (\colFelics, -5.75) {Candidate Interaction Groups};
...
\node[data] (final) at (\colShared, -28.5) {\textbf{Final Faithfulness Score ($\mathcal{F}(\mathbf{x})$)}};
